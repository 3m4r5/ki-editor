...
go to insert mode (indicated by 'INST' in the status bar) by clicking
...
(Esc) to go back to normal mode and click Enter to save."
...
selection mode, these are called movements, every selection mode uses the
same movements (positional coherence).
...
You might have noticed that in line selection mode {{l}} (>>) and {{j}}
(<<) skip empty lines, You can use {{u}} (<) and {{o}} (>) if you don't
want to skip empty lines, You can also use {{i}} (^) and {{k}} (v) to skip
to empty lines. You can click Backspace to go to previous selection, {{y}}
(|<) to go to the beginning and {{p}} (>|) to go to the end. To go to a
specific line click {{M}} (shift+{{m}}), type the line number and click
Enter. You can also use {{m}} (jump) and type the first character of the
line you want to jump to, if there's only one line that begins with that
character it will get selected, if there are multiple lines starting in
that character, each line will show a different character at the
beginning, click the character that is shown at the beginning of the line
to select it."
